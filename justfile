...
build-test-program-rust features=default-features:
    ...
    xcopy /y crates\\test_program_rust\\target\\release\\test_program_rust.exe outputs\\
...
build-exploit-network features=default-features:
...
generate-network features=default-features:
    just --justfile {{justfile()}} build-exploit-network {{features}}
